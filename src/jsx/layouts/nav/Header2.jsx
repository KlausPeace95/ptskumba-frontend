...
const Header2 = ({ onNote }) => {
	...
	// Auth info for header dropdown
	const { user, getUserRole } = useAuthStore();
	const roleLabel = (getUserRole && getUserRole()) ? getUserRole() : undefined;
	const displayName = user?.first_name || user?.username || user?.email || "User";
	const prettyRole = roleLabel ? roleLabel.charAt(0).toUpperCase() + roleLabel.slice(1) : "";
